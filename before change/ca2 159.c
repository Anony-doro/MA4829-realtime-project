...
void* waveform_thread(void* arg) {
    ...
    while(state.running) {
        pthread_mutex_lock(&mutex);
        current_freq = state.frequency;  // Get current frequency while locked
        
        if (mode == 2) {
            // Use potentiometer values in pot mode
            period_ns = (1000000000.0 / ((current_freq * pot_freq) * POINTS_PER_CYCLE));
            actual_amplitude = (short)state.data[point_index] * pot_amp;
        } else {
            // Use direct values in keyboard mode
            period_ns = (1000000000.0 / (current_freq * POINTS_PER_CYCLE));
            actual_amplitude = (short)state.data[point_index];
        }
        ...
        // Calculate and wait for next point time
        do {
