...
uint16_t adc_in;
double pot_frequency = 1;  // potentiometer reading for frequency
double pot_amplitude = 1;  // potentiometer reading for amplitude
unsigned int count;
unsigned short chan;
...
void* adc_thread(void* arg) {
    ...
    while(state.running) {
        ...
        if (count == 0x00) // Frequency control potentiometer
        {
            pot_frequency = (double)adc_in / 0xFFFF/2 + 0.5; // Scale to 0 to 1
            // Write to file
        ...
        if (count == 0x01) // Amplitude control potentiometer
        {
            pot_amplitude = 0.5 + 0.5 * (double)(adc_in-1.0) / 0xFFFF; // Scale to 0 to 1
            pot_amplitude *= (amplitude/2.5);
            // Write to file
        ...
        printf("Pot frequency: %.2f\n", pot_frequency * frequency);
        printf("Pot amplitude: %.2f\n", pot_amplitude * amplitude);
...
void* waveform_thread(void* arg) {
    ...
    while(state.running) {
        ...
        period_ns = (1000000000.0 / ((frequency * pot_frequency) * POINTS_PER_CYCLE));
        actual_amplitude = (short) state.data[point_index] * pot_amplitude;
...
int main(int argc, char* argv[]) {
    ...
    // if (access("default_settings.txt", F_OK) == 0) {
    //     printf("Loading default settings\n");
    //     load_settings("default_settings.txt");
    // }  // currently shows memory fault (core dumped)
