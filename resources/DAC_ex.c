...
//
// Note: Single ADC, Disable Burst, Single write to DAC, individual write seems to work.
...
//*****************************************************************************																				
printf("\nDIO Functions\n");													  										
out8(DIO_CTLREG,0x80);		// Port A : Output B; Input Port C : out|out 									
out8(DIO_PORTA,0x55);						// Write to Port A												
dio_in=in8(DIO_PORTA); printf("Port A : %02x\n",dio_in);	// Read back																
out8(DIO_PORTB,0xf0);						// Write to Port B  		
dio_in=in8(DIO_PORTB); printf("Port B : %02x\n",dio_in);	// Read back
												 										
out8(DIO_PORTC,0x0f);						// Write to Port C 															
